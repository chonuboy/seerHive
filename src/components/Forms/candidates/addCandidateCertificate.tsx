...
export default function AddCandidateCertificate({
  ...
  const handleCertificateSelect = async (certificateName: string) => {
    try {
      ...
      // Check if certificate already exists in candidate's certificates
      if (
        candidateCertificates.some(
          (cert) => cert.certification?.certificationName.toLowerCase() === certificateName.toLowerCase()
        )
      ) {
      ...
      // Check if certificate exists in master certificates
      const certificateExists = masterCertificates.some(
        (cert) => cert.certificationName.toLowerCase() === certificateName.toLowerCase()
      );

      let certificateToAdd;
      if (certificateExists) {
        // If certificate exists, find it in master certificates
        certificateToAdd = masterCertificates.find(
          (cert) => cert.certificationName.toLowerCase() === certificateName.toLowerCase()
        );
      } else {
        ...
        // Update master certificates with the new certificate
        // Note: You might need to pass setMasterCertificates as a prop if you want to update the parent state
        // masterCertificates = [...masterCertificates, createdCertificate];
        
        certificateToAdd = createdCertificate;
      }

      // Show the form to add certificate details
      setSearchTerm(certificateName);
      setIsDropdownOpen(false);
  ...
  const handleAddCertificate = async () => {
    try {
      ...
      // Check if certificate exists in master certificates (again in case it was added elsewhere)
      const certificateExists = masterCertificates.some(
        (cert) => cert.certificationName.toLowerCase() === searchTerm.toLowerCase()
      );
      ...
      if (certificateExists) {
        ...
        // Create the new certificate if it doesn't exist
        const newCertificate = {
          certificationName: searchTerm,
        };
        certificateToAdd = await createCertification(newCertificate);
      }
  ...
  const handleCertificateRemove = (certificateToRemove: contactCertificate) => {
    deleteContactCertification(certificateToRemove.contactCertificationId).then((res) => {
        ...
        <div className="relative">
          ...
          >
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter") {
                  handleCertificateSelect(searchTerm);
                }
              }}
              ...
              {filteredCertificates.length > 0 ? (
                filteredCertificates.map((certificate) => (
                  <div
                    ...
                    onClick={() => handleCertificateSelect(certificate.certificationName)}
                  >
        ...
        <div className="flex flex-wrap gap-4">
          {candidateCertificates.map((certificate) => (
            ...
            >
              ...
              <button onClick={() => handleCertificateRemove(certificate)} className="absolute -right-2 -top-2">
