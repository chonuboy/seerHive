...
export default function AddCandidateCompany({
  ...
  const handleSaveClick = (companyToUpdate: Companies) => {
    // Basic validation
    ...
    updateContactCompany(companyToUpdate.contactCompanyId, {
      ...
      .then(() => {
        setEditingCompanyId(null); // Exit editing mode
  ...
  const handleCompanySelect = (company: Company) => {
    if (
      ...
      createContactCompany({
        ...
        setCandidateCompanies([...candidateCompanies, res]);
        setSearchTerm("");
        setIsDropdownOpen(false);
      });
    }
  ...
  const handleCompanyRemove = (companyToRemove: Companies) => {
    deleteContactCompany(companyToRemove.contactCompanyId).then((res) => {
      const filtered = candidateCompanies.filter(
        (company) =>
          company.contactCompanyId !== companyToRemove.contactCompanyId
      );
      setCandidateCompanies(filtered);
    });
  };
      ...
      prevCompanies.map((company) =>
        company.contactCompanyId === companyId
          ? {
              ...company,
              [field]: value,
              // Special handling for isCurrentCompany: if checked, clear leavingDate
              ...(field === "isCurrentCompany" &&
                value === true && { leavingDate: null }),
            }
          : company
      )
    );
        ...
        <div className="flex flex-col gap-6">
          {candidateCompanies.map((company) => (
            <div
              ...
              className="flex flex-col gap-4 p-4 bg-white border-2 border-cyan-400 rounded-lg relative text-gray-700 font-medium"
            >
              ...
              {editingCompanyId === company.contactCompanyId ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label
                      htmlFor={`joiningDate-${company.contactCompanyId}`}
                    ...
                    <input
                      type="date"
                      id={`joiningDate-${company.contactCompanyId}`}
                  ...
                  <div className="col-span-full flex items-center gap-2">
                    <input
                      type="checkbox"
                      id={`isCurrentCompany-${company.contactCompanyId}`}
                      checked={company.isCurrentCompany === true}
                      onChange={(e) =>
                        handleFieldChange(
                          company.contactCompanyId,
                          "isCurrentCompany",
                    ...
                    />
                    <label
                      htmlFor={`isCurrentCompany-${company.contactCompanyId}`}
                  ...
                  <div className="col-span-full flex justify-end gap-2 mt-2">
                    <button
                      onClick={() => setEditingCompanyId(null)} // Cancel editing
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm">
                  ...
                  <div className="col-span-full flex justify-end mt-2">
                    <button
                      onClick={() => handleEditClick(company.contactCompanyId)}
                      className="px-4 py-2 border border-gray-300 text-gray-700 rounded-md hover:bg-gray-50 transition-colors duration-200"
                    >
                      Edit
                    </button>
