...
export default function Step3JobDescription() {
  ...
  //       const f = { name: file.name, size: file.size, url: r.url };
  //       handleInputChange("jd", f);
  //       jdFileHelpers.setValue(f);
  //     })
  //     .catch((e) =>
  //       toast.error(`Upload failed: ${e.message}`, { position: "top-right" })
  //     );
  // };
  const handleFileUpload = (file: File) => {
    uploadJD({
    ...
    <div className="space-y-8">
      ...
      <div>
        <QuillEditor
          ...
          height={300}
        />
        {jdMeta.touched && jdMeta.error && (
          <p className="mt-2 text-red-600 font-medium">{jdMeta.error}</p>
        )}
      </div>
